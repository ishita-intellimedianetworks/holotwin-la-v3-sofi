...
/** Walk-speed multiplier control (1× / 2× / 3× / 5×), shown while walking. Each
 *  walk starts at 1× (reset in navigateToPoint); tapping applies it live.
 *
...
export function SpeedControl({ ctrlRef, vertical = false }: { ctrlRef: RefObject<PlayerControllerHandle | null>; vertical?: boolean }) {
  ...
  // Keep in sync with the controller (cross-instance changes + per-walk reset).
  useEffect(() => {
