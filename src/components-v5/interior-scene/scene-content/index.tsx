...
import type { FloorConfig, FloorTransition, FurnitureConfig } from "@/components-v5/shared/types";
import { useInterior } from "../context/context";
import { PlayerController } from "./components/player-controller";
import { NavPath3D } from "./components/nav-path-3d";
import { HotspotMarkers } from "./components/hotspot-markers";
import { CrowdFlowMesh } from "./components/crowd-flow";
import { SingleModel } from "./components/model-load";
import {
  SingleNavmesh,
  type RoomZone,
} from "./components/navmesh/navmesh-geomtery";
import { zoneNameForFloor } from "./components/navmesh";
import SceneEnvironment from "./components/environment";
import DollhouseCamera from "./components/dollhouse-camera";
import { PerfMeter } from "./components/perf-meter";
import { useSceneLoading } from "./hooks/use-scene-loading";
import { setupFurnitureToggle } from "./components/model-load/furniture-swap";
...
import { HoloTwinPreview } from "@/components-v5/shared/ui/molecules/loading-screen/utils/core";
import type { SharedUniforms } from "@/components-v5/shared/ui/molecules/loading-screen/utils/core";
import { useProgressStore } from "@/components-v5/shared/store/progress-store";
import { useCameraStore } from "@/components-v5/shared/store/camera-store";
import { useWorldStore } from "@/components-v5/shared/store/world-store";
...
function ProgressSmoother() {
  const { isFurnitureToggleReady } = useInterior();
...
function CameraStoreBinder() {
  ...
  const setCameraRef = useCameraStore((s) => s.setCameraRef);
  useEffect(() => {
    setCameraRef(camera);
  }, [camera, setCameraRef]);
  return null;
...
export function SceneContent({
  ...
  return (
    <>
      ...
      {currentModelUrl && (
        <SingleModel
          ...
          onBounds={(bbox) => {
            ...
            const r = bbox.getSize(new THREE.Vector3()).length() * 0.5;
            setWorldBounds({ center: [c.x, c.y, c.z], radius: r });
            // Feed the minimap bounds (unless a dedicated boundsUrl model does).
            if (!(viewMode === "firstPerson" && activeFloor?.boundsUrl)) {
              modelCallbacksFor(currentModelKey).onBounds(bbox);
            }
          }}
      ...
      {/* Hotspot markers — any floor whose destinations carry hotspot positions
          (memorial/stadium). Mounted in BOTH views: in first person they track
          the open category; in the dollhouse (no panel there) they show every
      ...
      {/* Crowd-flow heatmap GLB (memorial) — zone meshes tinted by congestion
          tier, visible only while the Crowd Flow category is open. */}
      {activeFloor?.crowdFlowGlb && !activeFloor.interior && (
        <CrowdFlowMesh
          key={`crowd-${activeFloor.id}`}
          url={activeFloor.crowdFlowGlb.url}
          levels={activeFloor.crowdFlowGlb.levels}
        />
      )}
