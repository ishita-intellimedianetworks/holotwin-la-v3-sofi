...
/**
 * InteriorInlineProvider — owns interior state + lifecycle effects. Provides
 * InteriorContext (for SceneContent) and InteriorInlineUIContext (for overlays).
 */
...
import type { MinimapData } from "./minmap/types/types";
import { useFadeTransition } from "../shared/ui/molecules";
import {
  getSharedUniforms,
  resetSharedUniforms,
} from "@/components-v5/shared/ui/molecules/loading-screen/utils/core";
import { useProgressStore } from "../shared/store/progress-store";
import { useAppStore } from "../shared/store/app-store";
import { InteriorContext, type InteriorContextValue } from "./context/context";
import { tokens } from "@/components-v5/shared/tokens";
import {
  InteriorInlineUIContext,
  type InteriorInlineUI,
  type InteriorSceneContentData,
  type Phase,
} from "./inline-context";
...
export default function InteriorInlineProvider({
  nodeId = defaultApartmentId,
  inlineMode,
  active = true,
  ...
  const isInteriorInstructionsSeen = useAppStore((s) => s.isInteriorInstructionsSeen);
  ...
  const [phase, setPhase] = useState<Phase>(
    (!inlineMode && hasDollHouse)
      ? (isInteriorInstructionsSeen ? "dollhouse" : "overlay")
      : "firstPerson",
  );
  ...
  useEffect(() => {
    if (!inlineMode && dollHouseCamera && isInteriorInstructionsSeen) {
      setPhase("dollhouse");
    }
  ...
  // Plain data object for the R3F children, rebuilt each render. Passed through
  // context as a value (not a ref) so consumers can read it during render.
  const sceneContent: InteriorSceneContentData = {
  ...
  const uiValue: InteriorInlineUI = {
  ...
  return (
    <InteriorContext.Provider value={interiorContextValue}>
      <InteriorInlineUIContext.Provider value={uiValue}>
        {children}
      </InteriorInlineUIContext.Provider>
