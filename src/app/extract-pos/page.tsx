    ...
    loader.load(
      ...
      (gltf) => {
        ...
        gltf.scene.traverse((node: THREE.Object3D) => {
          ...
          // Decompose to a YXZ-order Euler — this is the order the runtime player
          // applies camera rotation in (x=pitch, y=yaw, z=roll; see
          // player-controller teleport.ts / use-walk-frame.ts, both
          // `camera.rotation.set(x, y, z, "YXZ")`). Emitting YXZ keeps the output
          // drop-in for scenes.json POI rotations: a level camera yields
          // [pitch, yaw, 0], never the flipped [±π, yaw', ±π] that XYZ produces
          // for angled cameras (which renders the view upside-down).
          worldEul.setFromQuaternion(worldQuat, "YXZ");
